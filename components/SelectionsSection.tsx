    ...
    <section id="selecciones" className="mx-auto max-w-[1400px] px-5 py-16 md:px-8 md:py-24">
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-3">
        {selections.map((sel) => (
          <Link
            key={sel.name}
            href={`/catalogo?team=${encodeURIComponent(sel.name)}`}
            className="block border border-line px-6 py-8 transition-colors hover:border-ink"
          >
            <JerseyVisual colors={sel.colors} alt={sel.name} className="mx-auto h-40 w-auto" />
